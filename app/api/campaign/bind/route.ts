...
export async function POST(request: Request) {
  try {
    const { twitterId, walletAddress, signature, messageBytes } =
      await request.json();
    ...
    try {
      ...
      if (publicKey.toSuiAddress() !== walletAddress) {
        return NextResponse.json(
          { error: "Signature invalid for this address" },
          { status: 401 },
        );
      }
    } catch (e) {
    ...
    if (existingTwitter && existingTwitter.walletAddress !== walletAddress) {
      return NextResponse.json(
        { error: "This Twitter is already bound to another wallet." },
        { status: 409 },
      );
    }

    // 檢查錢包是否已綁定其他 Twitter
    const existingWallet = usersDB.find(
      (u) => u.walletAddress === walletAddress,
    );
    if (existingWallet && existingWallet.twitterId !== twitterId) {
      return NextResponse.json(
        { error: "This Wallet is already bound to another Twitter." },
        { status: 409 },
      );
    }
